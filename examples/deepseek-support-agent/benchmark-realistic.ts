...
const scenarioPath = process.env.SCENARIO ?? "./examples/scenarios/deepseek-realistic-routing.yaml";
const target = "http://127.0.0.1:3002/api/agent";
const iterations = Number.parseInt(process.env.ITERATIONS ?? "3", 10);

if (!process.env.DEEPSEEK_API_KEY) {
  console.error("DEEPSEEK_API_KEY is required to run the benchmark.");
  process.exit(1);
}

const server = spawn(process.execPath, ["./examples/deepseek-support-agent/server-realistic.ts"], {
...
async function waitForServer(): Promise<void> {
  ...
  while (Date.now() < deadline) {
    if (serverOutput.includes("DeepSeek realistic agent listening")) {
      return;
    }
